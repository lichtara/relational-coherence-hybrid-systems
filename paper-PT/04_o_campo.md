...
## 4.4 Campos em Sistemas Híbridos Humano–IA

Em sistemas híbridos, os campos são coproduzidos por componentes humanos e artificiais.

Características centrais incluem:

- Mediação algorítmica da percepção, da atenção e da escolha
- Incorporação institucional de processos decisórios automatizados
- Amplificação por retroalimentação, impulsionada por otimização orientada por dados
- Opacidade e assimetria entre operadores do sistema e populações afetadas

O campo, portanto, não é estático; ele evolui à medida que modelos são reentreinados, políticas se alteram e comportamentos se adaptam.
...
Campo é uma condição necessária para compreender sistemas híbridos, mas não é suficiente por si só.

Um campo bem definido não garante estabilidade nem operação ética. Para que um sistema permaneça governável e sustentável, o campo deve ser acoplado a:

- Coerência — compatibilidade estrutural e inteligibilidade mútua dentro do campo
- Limite — fronteiras operacionais e institucionais que previnem sobrecarga sistêmica

Essas relações são formalizadas nas seções seguintes.

## 4.8 Transição para Coerência

Se Campo define onde as interações ocorrem, Coerência define como essas interações permanecem inteligíveis, compatíveis e sustentáveis ao longo do tempo.

A próxima seção introduz uma definição formal de Coerência como uma propriedade estrutural que emerge dentro — e é constrangida pelo — campo.
